...
$sql = $query === "id" ? "SELECT *
                          FROM users WHERE  users.blocked=?
                          ORDER BY users.id" : "SELECT *
                          FROM users WHERE  users.blocked=? ORDER BY users.username";
...
if (isset($_POST["unblock"])) {
    ...
    $stmt = $pdo->prepare($sql);
    $stmt->execute([false, $username]);
    header("location:blocked_users.php?success=user unblocked!");
...
?>
     <a href="blocked_users.php?q=alphebeticsort"><button class="btn btn-primary">sort  alphabetically</button></a><br>
  <a href="blocked_users.php"><button class="btn btn-primary">sort by  time</button></a>
...
if (count($users) > 0) {
    foreach ($users as $row) {
        ...
        echo '<tr>
         ...
         <td class="inheader">
         <form method="POST" action="blocked_users.php">
         <input type="text" name="username" style="display:none" value="' . $username . '">
         <input class="btn btn-success" type="submit" value="UnBlock" name="unblock">
         </form>
         </td>
        </tr>';
    }
    ...
    echo '<tr>
        <td colspan="3" align="center">No Blocked users found</td>
       </tr>';
}
?>
</tbody></table>
